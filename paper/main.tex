...
the model-level intervention is the additional retinal-domain MAE pretraining
stage that turns that initialisation into RETFound. As frozen feature extractors the
...
under identical partial fine-tuning (last 4 of 24 blocks) the advantage is no
longer statistically detectable on any held-out domain. A direct
difference-of-differences test of the protocol interaction is negative in
direction on two of three domains with every seed agreeing, but does not reach
significance at five seeds after correction; we report that rather than
inferring interaction from discordant significance. The point estimates therefore show substantial attenuation of the
ImageNet-MAE advantage after partial fine-tuning on DDR and APTOS, but a direct
test of the protocol interaction does not reach multiplicity-adjusted
significance at the current five-seed budget. We report that limit rather than
inferring an interaction from discordant significance, and we do not claim the
protocols have been shown to be equivalent either. We situate this within a broader evaluation of cross-domain DR grading on
...
risk minimisation on either target tested, A configuration change from 224\,px at batch 32 to 512\,px at batch 16 improves
held-out QWK by up to $0.097$, but that comparison alters resolution and batch
size together and we do not currently separate them. Across the study, three separate effects were statistically
...
other not (Section~\ref{sec:interaction}). Since linear probing is the standard inexpensive way to
...
\item A quantification of what \emph{does} move cross-domain performance: a
      ...
      change tested, though resolution and batch size are not yet separated
...
\input{../outputs/tables/table_interaction}
...
\textbf{This is not yet a resolution effect.} The two configurations differ in
resolution \emph{and} batch size, because 512\,px at batch 32 does not fit the
8\,GB budget these experiments were run under. Attributing the gain to
resolution alone would require a 224\,px/batch-16 control, which we have not
run. We therefore report it as a configuration effect and separate the two
factors in future work; the decomposition requires
224/32 vs 224/16 (batch), 224/16 vs 512/16 (resolution) and 224/32 vs 512/16
(combined). \todo{restore the resolution-specific claim once the 224/b16
control is complete}
