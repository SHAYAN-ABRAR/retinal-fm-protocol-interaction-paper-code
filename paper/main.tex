...
$+0.0459$ from batch and $+0.0421$ from resolution. The resolution contrast
survives Holm correction on both domains; the batch contrast survives on
neither, though its IDRiD estimate moves all three seeds in the same direction
and is better read as underpowered than as absent.

We therefore report resolution as the dominant term on EyePACS and as about
half the effect on IDRiD, and quote the batch-controlled estimates rather than
the confounded ones. Two limits remain. Three seeds is a coarse basis for the
seed-level resample, so these intervals are honest about seed uncertainty
without being precise about it. And the in-domain 512\,px runs have no
224\,px/batch-16 counterpart, so in-domain resolution is still a configuration
effect and is reported as one.

\input{../outputs/tables/table_configuration}
