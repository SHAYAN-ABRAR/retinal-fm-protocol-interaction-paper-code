...
initialisation, and the smallest domain favours RETFound. The cheap protocol and
...
(\num{229} training runs): no domain-generalization objective beats empirical
...
Frozen, the general-purpose initialisation is ahead on DDR
($+0.0702$, $2.42\times$ SD) and APTOS ($+0.1075$, $1.95\times$), both clearing
Section~\ref{sec:bars}; IDRiD is within seed noise. Fine-tuned, no target
favours it: DDR $+0.0217$ ($0.82\times$), APTOS $+0.0073$ ($0.42\times$), and
IDRiD $-0.0559$ ($1.25\times$) favours RETFound.

The mechanism is visible in the source--target split. Frozen, both models fit
...
Against in-domain models evaluated on identical test images, deployment to an
unseen domain costs $0.138$ QWK on DDR and $0.291$ on EyePACS, with severe
...
\todo{deployment-cost table: regenerate the 224\,px three-seed paired version;
the existing table\_deployment\_cost.tex is the 512\,px seed-42 one}
...
\item \textbf{IDRiD is the smallest target} (507 test images) and its
      fine-tuned result rests on three seeds at the time of writing, which is
      the sample size this paper argues is insufficient. \todo{update when
      seeds 3--4 land}
...
All four datasets are publicly available and were used under their respective
licences. No new patient data were collected and no identifiable information
was processed; this is a secondary analysis of de-identified public data and
required no additional ethical approval. \todo{confirm per-dataset licence
statements before submission}

\section*{Reproducibility}
All \num{229} runs are recorded in a machine-checked experiment registry; every
